	...
	async fetch(request, env, ctx): Promise<Response> {
		...
		if (path === '/api/virtualbox') {
			if (method === 'GET') {
				...
				const body=await request.json();
				const {type,name,visibility,userId}=initSchema.parse(body);
				
				const vb=await db.insert(schema.virtualbox).values({type,name,visibility,userId}).returning().get();

				const initStorageRequest = new Request(`https://storage.tpanesar2174.workers.dev/api/init`, {
				method: 'POST',
			...
			if(!user) {
				return new Response("No user associated with this email",{status :400});
			}
