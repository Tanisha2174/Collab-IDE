...
io.use(async (socket , next)=>{
    ...
    const{userId, virtualboxId, type}= parseQuery.data;

    const dbUser=await fetch(`https://database.tpanesar2174.workers.dev/api/user?id=${q.userId}`)
...
io.on("connection",async (socket)=>{
    ...
    socket.on("deleteFile", async (fileId:string, callback)=>{
        ...
        fs.unlink(path.join(dirName,file.id), function(err){
            if(err) throw err;
        });
    ...
    socket.on("renameFile", async (fileId:string, newName:string)=>{
        ...
        const newFileId= parts.slice(0,parts.length-1).concat([newName]).join("/");
    ...
    socket.on("createTerminal",({id}:{id:string})=>{
        ...
        const onData= pty.onData((data)=>{
            socket.emit("terminalResponse",{data});
        });
