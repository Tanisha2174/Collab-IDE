import { R2FileData, TFile, TFolder, R2Files, TFileData } from "./types";


const getVirtualboxFiles=async(id:string)=>{
  const virtualboxRes=await fetch(`https://storage.tpanesar2174.workers.dev/api?virtualboxId=${id}`)
...
const fetchFileContent=async(fileId:string):Promise<string>=>{
    try{
        const res= await fetch(`https://storage.tpanesar2174.workers.dev/api?fileId=${fileId}`);
