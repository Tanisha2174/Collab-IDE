...
export default function SidebarFile({ data,selectFile,handleRename , handleDeleteFile}: { 
  data: TFile 
  selectFile:(file:TTab)=>void;
  handleRename:(id:string, newName:string, oldName:string, type:"file"|"folder")=> boolean;
  ...
  return (
    <ContextMenu>
      <ContextMenuTrigger disabled={pendingDelete} onClick={()=>{
        ...
        w-full flex items-center h-7 px-1 hover:bg-secondary rounded-sm cursor-pointer transition-colors focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring">
      ...
      //   setEditing(true);
      // }} onClick={()=>selectFile({...data,saved:true})} className="w-full flex h-7 px-1 items-center transition-colors hover:bg-secondary rounded-sm  cursor-pointer focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring">
      //   <Image
      //     src={imgSrc}
      //     alt="File Icon"
      //     width={16}
